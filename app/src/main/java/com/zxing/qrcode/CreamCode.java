...
public class CreamCode{
    //二维码或条形码
    public static Bitmap createCodeBitmap(int QRCODE_WIDTH,int QRCODE_HEIGHT,String content,BarcodeFormat format) {
    ...
    public static Bitmap creamQRcodeLogo(int QRCODE_WIDTH,int QRCODE_HEIGHT,String content,Resources res,int id) {
            ...
            if(QRCODE_WIDTH != 0 && QRCODE_HEIGHT != 0 && content != null ) {
                qr = createCodeBitmap(QRCODE_WIDTH,QRCODE_HEIGHT,content,BarcodeFormat.QR_CODE);
            }
            if(id != 0) {
